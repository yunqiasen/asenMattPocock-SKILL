...
- `MattPocock-Frok` 是二开开发分支和唯一安装源，所有删减、调用编排和 Skill 微调都在这里完成。
- 不得在 `main` 开发，不得在 `MattPocock-Frok` 执行 `git pull upstream main` 或 `git merge main`。
...
仅手动 Skill：`grill-with-docs`、`wayfinder`、`setup-matt-pocock-skills`、`ask-matt`、`improve-codebase-architecture`。
