...
**安装源**:
提供 Skill 文件的仓库和分支。本项目开发期间使用 `MattPocock-Frok` 分支。
_Avoid_: 目标项目
...
**二开分支**:
本仓库的 `MattPocock-Frok`，保存删减后的 16 个 Skill、工作流编排和安装逻辑，也是对外安装源。
_Avoid_: 上游镜像
...
**选择性移植**:
上游更新后，只把已评估且适合本项目的单个 Skill 改动带入 `MattPocock-Frok`，不整体合并 `main`。
_Avoid_: 同步分支
